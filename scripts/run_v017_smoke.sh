...
OUT="${TMPDIR:-/tmp}/unirevlab-v017-smoke"
...
kotlinc \
  ...
  "$ROOT/app/src/main/java/org/unirevlab/security/analysis/ResourceTableResolver.kt" \
  "$ROOT/app/src/main/java/org/unirevlab/security/analysis/ManifestRuleEngine.kt" \
  "$ROOT/app/src/main/java/org/unirevlab/security/analysis/CrossRuntimeCorrelator.kt" \
  ...
  "$ROOT/test-corpus/smoke/V017CodegenResourcesIntentSmoke.kt" \
  -include-runtime -d "$OUT/v017.jar"
java -jar "$OUT/v017.jar" "$ROOT" "$OUT"
python3 - "$ROOT" "$OUT/static-analysis-report.json" <<'PY'
import json, sys
from pathlib import Path
from jsonschema import Draft202012Validator
root = Path(sys.argv[1]); report = json.loads(Path(sys.argv[2]).read_text())
