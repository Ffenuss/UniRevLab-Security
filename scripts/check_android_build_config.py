...
checks = [
    ...
    ("v0.48 preview versionCode", app, r"versionCode\s*=\s*62"),
    ("v0.48 purchase trust-chain versionName", app, r'versionName\s*=\s*"0\.48\.0-preview-purchase-trust-chain"'),
    ...
    ("CI APK SHA-256", workflow, r'sha256sum .*UniRevLab-Security-v0\.48\.0-purchase-trust-chain-debug\.apk'),
    ...
    ("full report streamed to disk", audit_worker, r'ReportJsonExporter\.write\(report, output, realDump\)'),
    ...
    ("analysis cache invalidated for v0.48", inspector, r'ENGINE_VERSION\s*=\s*"0\.48\.0-purchase-trust-chain"'),
    ...
    ("customer action map wired", audit_worker + customer_action_map, r'CustomerActionMapExporter\.build\(report\)[\s\S]*customer-action-map\.json|CUSTOMER_ACTION_MAP'),
