...
from modkit.mobile import (
    ...
    native_inventory, qml_deep, runtime_profiler, unreal_deep, wasm_deep,
)

SCHEMA = "modkit-embedded-analysis-1.8"
...
def run_workspace(
    ...
    _check(cb, "Embedded 1/18 · runtime / engine profiler…")
    try:
    ...
    _check(cb, "Embedded 2/18 · multi-runtime engine routing…")
    try:
    ...
    _check(cb, "Embedded 3/18 · deobfuscation / protection profile…")
    try:
    ...
    _check(cb, "Embedded 4/18 · artifact families…")
    try:
    ...
    _check(cb, "Embedded 5/18 · universal ELF / Android ABI inventory…")
    try:
    ...
    _check(cb, "Embedded 6/18 · .NET / Mono CLI metadata…")
    try:
    ...
    _check(cb, "Embedded 7/18 · Unreal cooked assets / reflection…")
    try:
    ...
    _check(cb, "Embedded 8/18 · Godot PCK / scene graph…")
    try:
    ...
    _check(cb, "Embedded 9/18 · Defold archive / resources…")
    try:
    ...
    _check(cb, "Embedded 10/18 · Qt / QML structure…")
    try:
    ...
    _check(cb, "Embedded 11/18 · JavaScriptCore / React Native…")
    try:
    ...
    _check(cb, "Embedded 12/18 · WebAssembly sections / exports…")
    try:
    ...
    _check(cb, "Embedded 13/18 · Lua bytecode…")
    try:
    ...
    _check(cb, "Embedded 14/18 · Hermes HBC…")
    try:
    ...
    _check(cb, "Embedded 15/18 · native ELF/ARM64…")
    try:
    ...
    _check(cb, "Embedded 16/18 · Cocos correlation…")
    try:
    ...
    _check(cb, "Embedded 17/18 · Flutter/Dart AOT…")
    try:
    ...
    _check(cb, "Embedded 18/18 · gameplay semantic correlation…")
    try:
