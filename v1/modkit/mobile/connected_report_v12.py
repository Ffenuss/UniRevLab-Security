"""Connected report 1.2: base report plus runtime and IL2CPP evidence.

Runtime procfs layout, metadata/ELF structural checks and metadata identity are
...
def _il2cpp_index(rows: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
    rank = {"STRUCTURAL_BOTH_PRESENT": 3, "ELF_EXECUTABLE_RVA_CONFIRMED": 2, "METADATA_METHOD_NAME_CONFIRMED": 1, "UNRESOLVED": 0}
...
def _ensure_il2cpp_crosscheck(root: Path) -> dict[str, Any]:
    ...
    summary_path, rows_path = root / "il2cpp-crosscheck.json", root / "il2cpp-crosscheck.methods.jsonl"
...
def _safe_identity(row: dict[str, Any]) -> dict[str, Any] | None:
    if bool(row.get("promotesBuildability")) or bool(row.get("actionable")) or bool(row.get("buildable")) or bool(row.get("addressConfirmed")):
        return None
    if row.get("rva") not in (None, "", 0, "0", "0x0"):
        return None
    ...
    if status in {"UNRESOLVED_NO_RVA", "METADATA_TOKEN_CONFLICT_NO_RVA"} or bool(row.get("metadataTokenConflict")):
        return None
    return {"engine":"il2cpp.metadata-identity-embedded","status":status,"class":row.get("class"),"methodName":row.get("methodName"),"metadataMethodNamePresent":bool(row.get("metadataMethodNamePresent")),"metadataQualifiedMethodPresent":bool(row.get("metadataQualifiedMethodPresent")),"metadataToken":row.get("metadataToken"),"metadataTokenConfirmed":bool(row.get("metadataTokenConfirmed")),"metadataTokenConflict":False,"metadataResolvedClass":row.get("metadataResolvedClass"),"metadataResolvedMethodName":row.get("metadataResolvedMethodName"),"addressConfirmed":False,"rva":None,"actionable":False,"buildable":False,"promotesBuildability":False}
...
def _safe_identity_conflict(row: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if bool(row.get("promotesBuildability")) or bool(row.get("actionable")) or bool(row.get("buildable")):
        return None
    return {"engine":"il2cpp.metadata-identity-embedded","status":"METADATA_TOKEN_CONFLICT_NO_RVA","class":row.get("class"),"methodName":row.get("methodName"),"metadataToken":row.get("metadataToken"),"metadataTokenConfirmed":False,"metadataTokenConflict":True,"metadataResolvedClass":row.get("metadataResolvedClass"),"metadataResolvedMethodName":row.get("metadataResolvedMethodName"),"addressConfirmed":False,"rva":None,"actionable":False,"buildable":False,"promotesBuildability":False}
