...
public class PatchPackActivity extends Activity {
    ...
    private void startWork(Intent i){if(app.busy.get()){toast("Сейчас выполняется другая операция");return;}if(!app.file("game.apk").isFile()){toast("На главном экране сначала выберите исходный APK");return;}if(!app.file("patchpack.zip").isFile()&&!"patchpack_import".equals(i.getStringExtra("op"))){toast("Сначала выберите ZIP");return;}app.cancelled.set(false);app.busy.set(true);app.progress("Подготовка…");i.setClass(this,WorkerService.class);startForegroundService(i);}
