...
final class FileFormatDetector {
    enum Kind { TEXT, JSON, XML, DEX, ELF, APK_ZIP, ZIP, AXML, ARSC, SQLITE, PNG, JPEG, WEBP, UNITY_BUNDLE, UNITY_ASSET, IL2CPP_METADATA, BINARY }
    ...
    static Result detect(String path,byte[] data){
        ...
        if(name.endsWith(".assets")||name.endsWith(".resource")||name.endsWith(".resS".toLowerCase(Locale.ROOT))||name.endsWith(".lpak")||name.contains("globalgamemanagers"))return r(Kind.UNITY_ASSET,"Unity asset/data",false,true,"Unity asset inspector");
        boolean text=looksText(data);if(text){String trimmed=new String(data,0,Math.min(n,4096),StandardCharsets.UTF_8).trim();if(trimmed.startsWith("{")||trimmed.startsWith("["))return r(Kind.JSON,"JSON text",true,false,"Text editor");if(trimmed.startsWith("<?xml")||trimmed.startsWith("<manifest")||trimmed.startsWith("<resources")||name.endsWith(".xml"))return r(Kind.XML,"XML text",true,false,"Text editor");return r(Kind.TEXT,"UTF-8 text",true,false,"Text editor");}
