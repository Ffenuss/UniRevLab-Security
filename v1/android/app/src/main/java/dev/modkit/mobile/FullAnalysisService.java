            ...
            try{
                ...
                if(cached){progress("Полная реконструкция: кэш hit · target не изменился.");}
                    ...
                    try(DecompilerEngine dec=new DecompilerEngine(this)){
                        dec.open();File zip=dec.exportAllZip(app.cancelled);
                    ...
                    }catch(Exception decompileError){state.put("complete",false).put("error",String.valueOf(decompileError.getMessage())).put("finishedAtMs",System.currentTimeMillis());progress("Полная реконструкция частична: "+decompileError.getMessage()+" · продолжаю остальные анализаторы.");}
                    Files.write(manifest.toPath(),state.toString(2).getBytes(StandardCharsets.UTF_8));
                }
