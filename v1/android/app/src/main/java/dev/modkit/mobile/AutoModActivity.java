...
public class AutoModActivity extends AppCompatActivity {
    ...
    @Override public void onCreate(Bundle state){
        ...
        TextView note=text("Evidence Graph сам раскладывает находки по готовности. Runtime load-base/RVA, IL2CPP metadata+ELF cross-check и no-RVA metadata identity усиливают доказательства, но не делают finding buildable. Подтверждённый Class::Method без RVA остаётся REVIEW и не получает выдуманный адрес. В сборку проходят только локальные app-owned controls с validated executable binding и успешным preflight.",13);note.setTextColor(muted());root.addView(note);
    ...
    private void rebuildPlan(){
        ...
        planning=true;refresh.setEnabled(false);status.setText("AutoMod: Evidence Graph + runtime + IL2CPP structural + metadata identity…");
        executor.execute(()->{
            try{
    ...
    private void render(){
        ...
        summary.setText("Готово к build: "+count(plan,"readyToBuildCount")+" · preflight: "+count(plan,"readyForPreflightCount")+" · runtime/binding: "+count(plan,"runtimeNeededCount")+" · review: "+count(plan,"reviewCount")+" · audit-only: "+count(plan,"auditOnlyCount")+" · исключено: "+count(plan,"excludedCount")+"\nExact locators: "+count(plan,"exactLocatorCount")+" · runtime VA observed: "+count(plan,"runtimeObservedCount")+" · IL2CPP structural: "+count(plan,"il2cppStructuralObservedCount")+" (both "+count(plan,"il2cppStructuralBothCount")+") · metadata identity/no-RVA: "+count(plan,"metadataIdentityObservedCount")+" (qualified "+count(plan,"metadataQualifiedNoRvaCount")+") · всего: "+count(plan,"candidateCount"));
    ...
    private void candidateCard(JSONObject row){
        ...
        JSONObject identity=row.optJSONObject("metadataIdentity");String identityText="";if(identity!=null){identityText=" · metadata "+(identity.optBoolean("metadataQualifiedMethodPresent")?"Class::Method":"method-name")+" / RVA unresolved";}
        TextView meta=text(row.optString("stage")+" · "+row.optString("verificationStage","FOUND_STATIC")+(domain.isEmpty()?"":" · "+domain)+locator+runtimeText+il2cppText+identityText,12);meta.setTextColor(muted());box.addView(meta);
