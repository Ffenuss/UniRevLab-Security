...
public class AutoModActivity extends AppCompatActivity {
    ...
    private void rebuildPlan(){
        ...
        executor.execute(()->{try{if(!Python.isStarted())Python.start(new AndroidPlatform(this));PyObject result=Python.getInstance().getModule("modkit.mobile.automod_cancellable").callAttr("build_workspace_plan",getFilesDir().getPath(),app.file("automod-plan.json").getPath(),new PlanningProgress());new JSONObject(result.toString());runOnUiThread(()->{status.setText("AutoMod-план обновлён. Старый prepare/preflight инвалидирован.");render();});}catch(Exception e){runOnUiThread(()->{status.setText("AutoMod: "+e.getMessage());toast("Не удалось обновить план");});}finally{planning=false;runOnUiThread(()->refresh.setEnabled(!app.busy.get()));}});
    }
    ...
    private void render(){
        JSONObject plan=read("automod-plan.json");JSONObject pf=read("menu-preflight.json");boolean auditReady=exactPrepareAuditReady();if(plan==null){summary.setText("План ещё не построен. Выполните полный анализ и нажмите «Обновить AutoMod-план».");candidates.removeAllViews();setButtons(null,pf);return;}
        ...
        if(pf==null)preflight.setText("Preflight ещё не выполнен. Сборка заблокирована.");else preflight.setText("Exact audit: "+(auditReady?"READY":"MISSING/BLOCK")+" · Preflight: "+(pf.optBoolean("readyForAutoBuild")?"READY":"BLOCK")+" · controls "+pf.optInt("controlCount",pf.optInt("controls"))+" · blockers "+pf.optInt("blockerCount",pf.optInt("blockers")));
