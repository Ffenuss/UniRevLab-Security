...
public class FileWorkspaceActivity extends Activity {
    ...
    private boolean hexMode=false,suppressEditorChange=false,preparedPatchStale=false;
    ...
    @Override public void onCreate(Bundle state){super.onCreate(state);app=(App)getApplication();
        ...
        editor.addTextChangedListener(new android.text.TextWatcher(){public void beforeTextChanged(CharSequence s,int a,int b,int c){}public void onTextChanged(CharSequence s,int a,int b,int c){}public void afterTextChanged(android.text.Editable e){if(!suppressEditorChange)invalidatePreparedPatch("Текущие правки изменились после подготовки Patch Pack.");}});
    ...
    private File patchStaleMarker(){return app.file("workspace-patch-stale");}
    private boolean hasPreparedPatch(){return app.file("workspace-patch.zip").isFile()||app.file("workspace-source.json").isFile();}
    private boolean patchIsStale(){return preparedPatchStale||patchStaleMarker().isFile();}
    private void writePatchStaleMarker(String reason){preparedPatchStale=true;try{Files.write(patchStaleMarker().toPath(),(System.currentTimeMillis()+"\n"+reason).getBytes(StandardCharsets.UTF_8));}catch(Exception e){AnalysisJournal.exception(this,"WORKSPACE_PATCH_STALE_MARK_FAILED",e);try{Files.deleteIfExists(app.file("workspace-patch.zip").toPath());Files.deleteIfExists(app.file("workspace-source.json").toPath());}catch(Exception ignored){}}}
    private void invalidatePreparedPatch(String reason){if(!hasPreparedPatch()&&!patchStaleMarker().isFile())return;writePatchStaleMarker(reason);if(!app.busy.get())try{Files.deleteIfExists(app.file("workspace-patch.zip").toPath());Files.deleteIfExists(app.file("workspace-patch.zip.tmp").toPath());Files.deleteIfExists(app.file("workspace-source.json").toPath());Files.deleteIfExists(app.file("workspace-report.json").toPath());}catch(Exception e){AnalysisJournal.exception(this,"WORKSPACE_PATCH_STALE_CLEANUP_FAILED",e);}if(status!=null)status.setText(reason+" Подготовьте Patch Pack заново.");updateButtons();}
    private void beginPatchTransaction(String reason)throws Exception{writePatchStaleMarker(reason);if(!patchIsStale())throw new IOException("Не удалось заблокировать устаревший Patch Pack");}
    private void finishPatchTransaction()throws Exception{Files.deleteIfExists(patchStaleMarker().toPath());preparedPatchStale=false;}

    private void updateButtons(){boolean opened=original.length>0;save.setEnabled(opened);export.setEnabled(opened);boolean apk=opened&&sourceApk!=null&&targetEntry!=null;patch.setEnabled(apk&&!app.busy.get());build.setEnabled(apk&&app.file("workspace-patch.zip").isFile()&&app.file("workspace-source.json").isFile()&&!patchIsStale()&&!app.busy.get());specialized.setVisibility(opened&&format!=null&&specialRouteSupported(format.kind)?View.VISIBLE:View.GONE);if(opened&&format!=null)specialized.setText("Открыть: "+format.route);}
    ...
    private void setOpened(byte[] data,String name,File apk,String entry,String openedStatus)throws Exception{invalidatePreparedPatch("Открыт новый файл или APK entry.");displayName=name;sourceApk=apk;targetEntry=entry;original=data;sourceEntrySha256=apk!=null&&entry!=null?sha256(data):"";format=FileFormatDetector.detect(entry==null?name:entry,data);hexMode=format.defaultHex;render();status.setText(openedStatus+"\nФормат: "+format.label+" · маршрут: "+format.route+(patchIsStale()?"\nПредыдущий Patch Pack устарел; подготовьте его заново.":""));AnalysisJournal.append(this,"FILE_OPEN","Workspace file opened",new JSONObjectSafe().put("name",name).put("format",format.kind.name()).put("bytes",data.length).put("sourceEntrySha256",sourceEntrySha256).json());}
    ...
    private void render(){if(original.length==0)return;if(format==null)format=FileFormatDetector.detect(targetEntry==null?displayName:targetEntry,original);renderedEditorText=hexMode?toHex(original):new String(original,StandardCharsets.UTF_8);suppressEditorChange=true;try{editor.setText(renderedEditorText);}finally{suppressEditorChange=false;}if(hexMode)modeButton.setText("Режим: HEX · "+format.label);else modeButton.setText("Режим: UTF-8 · "+format.label);String inspection=BinaryFormatInspector.inspect(format,original,displayName);meta.setText(displayName+" · "+original.length+" bytes"+(targetEntry==null?"":"\nAPK: "+sourceApk.getName()+"\nEntry: "+targetEntry)+"\n"+inspection);updateButtons();}
    ...
    private void switchMode(){if(original.length==0)return;try{byte[] current=editedBytes();original=current;format=FileFormatDetector.detect(targetEntry==null?displayName:targetEntry,current);hexMode=!hexMode;render();status.setText("Режим изменён без потери текущих правок. Исходный APK entry остаётся привязан отдельным SHA-256."+(patchIsStale()?" Patch Pack нужно подготовить заново.":""));}catch(Exception e){toast("Не удалось переключить режим: "+e.getMessage());}}
    private void saveWorking(){try{byte[] b=editedBytes();Files.write(app.file("workspace-edit.bin").toPath(),b);original=b;format=FileFormatDetector.detect(targetEntry==null?displayName:targetEntry,b);renderedEditorText=editor.getText()==null?"":editor.getText().toString();meta.setText(displayName+" · "+b.length+" bytes · рабочая копия сохранена\n"+BinaryFormatInspector.inspect(format,b,displayName));status.setText("Рабочая копия сохранена. SHA-256 исходного APK entry сохранён отдельно и не меняется."+(patchIsStale()?" Patch Pack нужно подготовить заново.":""));updateButtons();}catch(Exception e){toast("Ошибка: "+e.getMessage());}}
    ...
    private void writeEdited(Uri uri){try(OutputStream out=getContentResolver().openOutputStream(uri,"w")){if(out==null)throw new IOException("openOutputStream returned null");out.write(editedBytes());out.flush();status.setText("Изменённый файл экспортирован."+(patchIsStale()?" Patch Pack нужно подготовить заново.":""));}catch(Exception e){deleteCreatedDocument(uri);toast(e.getMessage());}}
    ...
    private void preparePatch(){try{
        if(sourceApk==null||targetEntry==null)throw new IOException("Откройте entry из APK");if(sourceEntrySha256.isEmpty())throw new IOException("SHA-256 исходного APK entry отсутствует; откройте entry заново");beginPatchTransaction("Patch Pack пересобирается из текущих правок.");TargetResolver.Target target=TargetResolver.resolve(app);JSONObject verification=TargetResolver.requireVerified(target,app.cancelled);TargetResolver.Member member=sourceMember(target);byte[] b=editedBytes();String originalSha=sourceEntrySha256,editedSha=sha256(b);
        ...
        Files.move(tmp.toPath(),zip.toPath(),StandardCopyOption.REPLACE_EXISTING);writeAtomicJson(app.file("workspace-source.json"),binding);finishPatchTransaction();AnalysisJournal.append(this,"WORKSPACE_PATCH_READY","File Workspace patch bound to canonical target",binding);
        startWork(new Intent().putExtra("op","workspace_inspect").putExtra("source",sourceApk.getCanonicalPath()));status.setText("Patch Pack подготовлен из текущих правок и привязан к target SHA; запускаю проверку совместимости…");updateButtons();
    }catch(Exception e){writePatchStaleMarker("Подготовка Patch Pack завершилась ошибкой.");AnalysisJournal.exception(this,"WORKSPACE_PATCH_FAILED",e);updateButtons();toast(e.getMessage());}}

    private void buildPatched(){if(patchIsStale()){toast("Patch Pack устарел после изменения файла. Подготовьте его заново.");return;}if(!app.file("workspace-patch.zip").isFile()||!app.file("workspace-source.json").isFile()){toast("Сначала подготовьте Patch Pack");return;}boolean set=isInstalledSet();String title=set?"modkit-workspace-test.apks":"modkit-workspace-test.apk";startActivityForResult(new Intent(Intent.ACTION_CREATE_DOCUMENT).setType(set?"application/zip":"application/vnd.android.package-archive").addCategory(Intent.CATEGORY_OPENABLE).putExtra(Intent.EXTRA_TITLE,title),BUILD_APK);}
    ...
    private void startBuild(Uri uri){
        ...
        if(patchIsStale()){deleteCreatedDocument(uri);toast("Patch Pack устарел после изменения файла. Подготовьте его заново.");return;}
    ...
    private void refreshStatus(){if(revision==app.revision)return;revision=app.revision;if(patchIsStale())status.setText("Patch Pack устарел после изменения текущего файла. Подготовьте Patch Pack заново.");else if(app.status!=null&&!app.status.isEmpty())status.setText(app.status);updateButtons();}
