...
public class FileWorkspaceActivity extends Activity {
    ...
    private void preparePatch(){try{
        if(sourceApk==null||targetEntry==null)throw new IOException("Откройте entry из APK");TargetResolver.Target target=TargetResolver.resolve(app);JSONObject verification=TargetResolver.requireVerified(target,app.cancelled);TargetResolver.Member member=sourceMember(target);byte[] b=editedBytes();String originalSha=sha256(original),editedSha=sha256(b);
