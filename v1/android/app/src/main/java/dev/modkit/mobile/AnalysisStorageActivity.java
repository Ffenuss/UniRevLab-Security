...
public class AnalysisStorageActivity extends AppCompatActivity {
    ...
    private String label(String name){
        if("automod-plan.json".equals(name))return "AutoMod readiness plan · "+name;
        if("runtime-session.json".equals(name))return "Read-only runtime snapshot · "+name;
        if("runtime-correlation.json".equals(name))return "RVA → runtime VA correlation · "+name;
        if("il2cpp-crosscheck.json".equals(name))return "IL2CPP metadata + ELF structural cross-check · "+name;
        if("il2cpp-crosscheck.methods.jsonl".equals(name))return "IL2CPP per-method structural evidence · "+name;
