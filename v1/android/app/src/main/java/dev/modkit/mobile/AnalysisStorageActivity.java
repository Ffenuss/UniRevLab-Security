...
public class AnalysisStorageActivity extends AppCompatActivity {
    ...
    private void render(){files.removeAllViews();String[] names={"full-reconstruction.json","modkit-decompiled.zip","apktool-analysis.json","apktool-workspace","artifact-families.json","embedded-analysis.json","analysis.summary.json","analysis.methods.jsonl","analysis.fields.jsonl","analysis.evidence-graph.jsonl","analysis.gameplay-coverage.json","re-analysis.json","security-surfaces.json","simple-catalog.json","rodroid","analysis-deep","menu-spec.json","menu-preflight.json","connected-report.json","connected-report.md"};int found=0;for(String n:names){File f=app.file(n);if(!f.exists())continue;TextView t=text("✓ "+n+" · "+human(size(f)),13);t.setTextColor(fg());files.addView(t);found++;}if(found==0){TextView t=text("Пока пусто. Сначала выполните полный анализ.",14);t.setTextColor(muted());files.addView(t);}}
}
