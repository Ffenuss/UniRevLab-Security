...
public class ReportCenterActivity extends AppCompatActivity {
    ...
    @Override protected void onActivityResult(int request,int result,Intent data){super.onActivityResult(request,result,data);if(request!=SAVE_BUNDLE||result!=RESULT_OK||data==null||data.getData()==null)return;Uri uri=data.getData();progress.setVisibility(View.VISIBLE);status.setText("Связываем методы, deep evidence и формируем Evidence Bundle…");worker.execute(()->{try{JSONObject connected=buildConnected();JSONObject manifest=EvidenceBundleExporter.export(this,uri);main.post(()->{progress.setVisibility(View.GONE);status.setText("Готово · связанных находок: "+connected.optInt("findingCount")+" · exact links: "+connected.optInt("exactLinked")+" · deep backend'ов с evidence: "+availableDeep(connected)+" · файлов в пакете: "+manifest.optInt("evidenceFileCount")+" · данных: "+manifest.optLong("evidenceBytes")+" байт.");});}catch(Exception e){main.post(()->{progress.setVisibility(View.GONE);status.setText("Ошибка экспорта: "+e.getMessage());});}});}
