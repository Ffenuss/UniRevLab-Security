...
public class ReportCenterActivity extends AppCompatActivity {
    ...
    @Override public void onCreate(Bundle state){super.onCreate(state);ScrollView scroll=new ScrollView(this);LinearLayout root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setPadding(dp(18),dp(18),dp(18),dp(28));root.setBackgroundColor(bg());scroll.addView(root);setContentView(scroll);TextView title=text("Отчёт полного анализа",28);title.setTypeface(null,android.graphics.Typeface.BOLD);root.addView(title);TextView note=text("Для SUCCESS/PARTIAL ModKit перед экспортом строит connected-report 1.2: связывает findings с method id / class / method / RVA, deep coverage, runtime RVA→VA corroboration и IL2CPP metadata+ELF evidence. Для FAILED/CANCELLED экспортируется diagnostic-only bundle текущего epoch без синтеза нового connected-report из старого cache evidence.",14);note.setTextColor(muted());root.addView(note);exportButton=new MaterialButton(this);exportButton.setText("Экспортировать связанный отчёт ZIP");exportButton.setAllCaps(false);exportButton.setOnClickListener(v->pick());root.addView(exportButton);cancelExport=new MaterialButton(this);cancelExport.setText("Отменить экспорт");cancelExport.setAllCaps(false);cancelExport.setEnabled(false);cancelExport.setOnClickListener(v->cancelExport());root.addView(cancelExport);progress=new ProgressBar(this);progress.setVisibility(View.GONE);root.addView(progress);status=text("Готово к экспорту.",14);status.setTextIsSelectable(true);root.addView(status);}
