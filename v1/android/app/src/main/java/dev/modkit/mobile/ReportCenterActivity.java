...
public class ReportCenterActivity extends AppCompatActivity {
    ...
    private JSONObject buildConnected() throws Exception{if(!Python.isStarted())Python.start(new AndroidPlatform(this));return new JSONObject(Python.getInstance().getModule("modkit.mobile.connected_report_streaming").callAttr("build_connected_report",getFilesDir().getPath(),new java.io.File(getFilesDir(),"connected-report.json").getPath(),new java.io.File(getFilesDir(),"connected-report.md").getPath()).toString());}
