...
public class AutoAnalysisActivity extends AppCompatActivity {
    ...
    private final Handler handler=new Handler(Looper.getMainLooper());private boolean targetPickerLaunched=false;private long lastRevision=-1;
    ...
    @Override public void onCreate(Bundle state){super.onCreate(state);app=(App)getApplication();ScrollView scroll=new ScrollView(this);root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setPadding(dp(16),dp(18),dp(16),dp(28));root.setBackgroundColor(bg());scroll.addView(root);setContentView(scroll);
        ...
        LinearLayout result=card("Результат");summary=text("Анализ ещё не завершён.",14);result.addView(summary);findings=new LinearLayout(this);findings.setOrientation(LinearLayout.VERTICAL);result.addView(findings);storage=button("Открыть результаты и хранилище",result,v->startActivity(new Intent(this,AnalysisStorageActivity.class)));mod=button("Подготовить мод из подтверждённых изменений",result,v->startActivity(new Intent(this,MenuBuilderActivity.class).putExtra("focus","autopilot")));
        handler.post(poll);
    ...
    private void openTarget(){if(app.busy.get()){toast("Сначала дождитесь завершения или отмените текущую операцию");return;}targetPickerLaunched=true;startActivityForResult(new Intent(this,TargetSelectionActivity.class),TARGET);}
    ...
    @Override protected void onActivityResult(int req,int result,Intent data){super.onActivityResult(req,result,data);if(req==TARGET&&result==RESULT_OK){targetPickerLaunched=false;startFullAnalysis();}}
