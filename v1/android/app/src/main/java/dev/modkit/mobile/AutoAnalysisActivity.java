...
public class AutoAnalysisActivity extends AppCompatActivity {
    ...
    private String pipelineBanner(JSONObject run,boolean busy){if(run==null||busy)return "";String state=run.optString("status","");if("PARTIAL".equals(state)){JSONArray reasons=run.optJSONArray("degradedReasons");return "Последний прогон завершён частично"+(reasons==null?"":" · причин: "+reasons.length())+".";}if("FAILED".equals(state))return "Последний прогон не завершён: "+run.optString("error","неизвестная ошибка")+".";if("CANCELLED".equals(state))return "Последний прогон отменён пользователем.";if("RUNNING".equals(state))return "Предыдущий прогон был прерван до фиксации финального результата.";return "";}
    private boolean catalogFromCurrentRun(JSONObject run){if(run==null)return false;long started=run.optLong("startedAtMs",0L),modified=app.file("simple-catalog.json").lastModified();return started>0L&&modified>=started;}
    private boolean autoModAllowedForRun(JSONObject run,boolean current){if(run==null||!current)return false;String state=run.optString("status","");return "SUCCESS".equals(state)||"PARTIAL".equals(state);}
    private long liveElapsedMs(JSONObject run,JSONObject progressRow){
        long started=0L;
        if(run!=null)started=run.optLong("startedAtMs",0L);
        if(started<=0L&&progressRow!=null){long snapshot=progressRow.optLong("elapsedMs",0L),updated=progressRow.optLong("updatedAtMs",0L);if(updated>0L&&snapshot>=0L)started=updated-snapshot;}
        return started>0L?Math.max(0L,System.currentTimeMillis()-started):(progressRow==null?0L:progressRow.optLong("elapsedMs",0L));
    }
    private void renderCatalog(JSONObject cat,JSONObject run){findings.removeAllViews();if(cat==null){summary.setText(app.busy.get()?"Анализ выполняется…":"Результаты ещё не сформированы.");mod.setEnabled(false);return;}boolean current=catalogFromCurrentRun(run);int total=cat.optInt("total"),important=cat.optInt("important"),ready=cat.optInt("buildable"),actionable=cat.optInt("actionable"),server=cat.optInt("serverAudit");String prefix=app.busy.get()?"Предыдущий сохранённый результат — текущий анализ ещё выполняется.\n":!current?"Показан предыдущий сохранённый каталог — текущий прогон не сформировал новый каталог.\n":"";summary.setText(prefix+"Найдено: "+total+" · важных: "+important+" · точных locator: "+actionable+" · готовых локальных изменений: "+ready+" · server/trust audit: "+server);mod.setEnabled(!app.busy.get()&&autoModAllowedForRun(run,current)&&(ready>0||actionable>0||important>0));JSONArray cards=cat.optJSONArray("cards");if(cards==null)return;int shown=0;for(int pass=0;pass<2&&shown<35;pass++){for(int i=0;i<cards.length()&&shown<35;i++){JSONObject c=cards.optJSONObject(i);if(c==null)continue;boolean noisy=c.optBoolean("lowSignal")||"FRAMEWORK_NOISE".equals(c.optString("ownership"));boolean audit=c.optBoolean("serverAudit");if(pass==0&&(noisy||audit))continue;if(pass==1&&!audit)continue;TextView row=text((c.optBoolean("buildable")?"READY · ":c.optBoolean("actionable")?"LOCATOR · ":"")+c.optString("title","Находка")+"\n"+c.optString("description",c.optString("status","")),14);row.setPadding(dp(10),dp(9),dp(10),dp(9));row.setBackgroundColor(surface());row.setOnClickListener(v->new android.app.AlertDialog.Builder(this).setTitle(c.optString("title","Находка")).setMessage(c.toString()).setPositiveButton("OK",null).show());findings.addView(row,new LinearLayout.LayoutParams(-1,-2));shown++;}}}
    private void refresh(){target.setText("Target: "+targetText());boolean busy=app.busy.get();progress.setVisibility(busy?View.VISIBLE:View.GONE);cancel.setEnabled(busy&&!app.cancelled.get());change.setEnabled(!busy);rerun.setEnabled(!busy&&(app.file("game.apk").isFile()||app.file("installed-target.json").isFile()));storage.setEnabled(!busy);String s=app.status==null?"":app.status;JSONObject run=json("automatic-evidence.json");String banner=pipelineBanner(run,busy);if(!banner.isEmpty())s=banner+(s.isEmpty()?"":"\n"+s);JSONObject p=json("simple-progress.json");if(p!=null&&busy){long live=liveElapsedMs(run,p);s+="\n"+phase(p)+" · этап "+p.optInt("stage")+"/"+p.optInt("totalStages")+" · "+p.optString("name")+" · прошло: "+elapsed(live)+" · осталось стадий: "+p.optInt("remainingStages");long heartbeat=p.optLong("updatedAtMs",0L);if(heartbeat>0L)s+=" · heartbeat: "+elapsed(Math.max(0L,System.currentTimeMillis()-heartbeat))+" назад";}status.setText(s);renderCatalog(json("simple-catalog.json"),run);}
