...
/**
 * Copies/records the selected target only. It deliberately performs no DEX/native/IL2CPP/security
 * analysis: the complete analysis starts exactly once in FullAnalysisService after selection.
 */
public class TargetPreparationService extends Service {
    private static final int NOTE_ID=94;
    private App app;
    ...
    @Override public void onCreate(){
        ...
        ((NotificationManager)getSystemService(NOTIFICATION_SERVICE)).createNotificationChannel(
                new NotificationChannel("target-preparation","Подготовка target",NotificationManager.IMPORTANCE_LOW));
    }
    @Override public IBinder onBind(Intent intent){return null;}

    private Notification note(String text){
        PendingIntent open=PendingIntent.getActivity(this,NOTE_ID,new Intent(this,TargetSelectionActivity.class),PendingIntent.FLAG_UPDATE_CURRENT|PendingIntent.FLAG_IMMUTABLE);
        return new Notification.Builder(this,"target-preparation").setSmallIcon(R.drawable.ic_modkit).setContentTitle("ModKit · выбор target").setContentText(text).setContentIntent(open).setOngoing(true).build();
    }
    ...
    @Override public int onStartCommand(Intent intent,int flags,int startId){
        ...
        new Thread(()->{
            try{
                ...
                if("installed".equals(kind)) prepareInstalled(intent.getStringExtra("package"),intent.getStringExtra("label"));
                else if("apk".equals(kind)) prepareApk(intent.getStringExtra("uri"),intent.getStringExtra("display"));
                else throw new IllegalArgumentException("Неизвестный тип target");
            }catch(Exception e){
        ...
        },"modkit-target-preparation").start();
    ...
    private void prepareInstalled(String packageName,String label)throws Exception{
        ...
        progress("Копирую APK-set установленного приложения…");
        PackageManager pm=getPackageManager();
        ApplicationInfo ai=pm.getApplicationInfo(packageName,0);
        PackageInfo pi=pm.getPackageInfo(packageName,0);
        List<File> sources=new ArrayList<>();
        sources.add(new File(ai.sourceDir));
        if(ai.splitSourceDirs!=null)for(String path:ai.splitSourceDirs)if(path!=null&&!path.isEmpty())sources.add(new File(path));
        for(File source:sources)if(!source.isFile())throw new java.io.FileNotFoundException(source.getAbsolutePath());

        clearTargetDependentOutputs();
        File dir=app.file("installed-apks");deleteTree(dir);if(!dir.mkdirs()&&!dir.isDirectory())throw new java.io.IOException("Не удалось создать installed-apks");
        ...
        for(int i=0;i<sources.size();i++){
            ...
            String name=i==0?"base.apk":String.format(Locale.ROOT,"split-%03d-%s",i,safeName(sourceName));
            File dest=new File(dir,name);copy(source,dest);
            splits.put(new JSONObject().put("index",i).put("name",name).put("path",dest.getCanonicalPath()).put("size",dest.length()).put("sha256",sha256(dest)));
            progress("Копирую APK-set: "+(i+1)+"/"+sources.size());
        }
        copy(new File(dir,"base.apk"),app.file("game.apk"));
        JSONObject target=new JSONObject()
                .put("schema","modkit-target-selection-1.0")
                .put("preparedOnly",true)
                .put("analysisPerformed",false)
    ...
    private void prepareApk(String uriText,String display)throws Exception{
        ...
        progress("Копирую выбранный APK…");
        Uri uri=Uri.parse(uriText);
        clearTargetDependentOutputs();
        File installed=app.file("installed-apks");deleteTree(installed);
        app.file("installed-target.json").delete();
        File temp=app.file("game.apk.part");temp.delete();
        try(InputStream in=getContentResolver().openInputStream(uri);FileOutputStream out=new FileOutputStream(temp)){
            if(in==null)throw new java.io.FileNotFoundException("Не удалось открыть APK");
            byte[] buf=new byte[1024*1024];int n;while((n=in.read(buf))!=-1){if(app.cancelled.get())throw new java.io.InterruptedIOException("cancelled");out.write(buf,0,n);}
        }
        if(temp.length()==0)throw new java.io.IOException("Выбран пустой APK");
        File dest=app.file("game.apk");if(dest.exists()&&!dest.delete())throw new java.io.IOException("Не удалось заменить game.apk");
        if(!temp.renameTo(dest)){copy(temp,dest);temp.delete();}
        getSharedPreferences("state",0).edit().remove("installed.package").putString("game.apk",display==null?"target.apk":display).apply();
        progress("Target готов: "+(display==null?"APK":display)+" · SHA-256 "+sha256(dest).substring(0,16)+"… · анализ ещё не запускался.");
    }
    ...
    private void clearTargetDependentOutputs(){
        String[] names={
                ...
                "artifact-families.json","embedded-analysis.json","automatic-evidence.json","automod-plan.json","runtime-session.json","runtime-correlation.json",
                "il2cpp-crosscheck.json","il2cpp-crosscheck.methods.jsonl","lua-deep.json","hermes-deep","hermes-deep.json",
                "native-deep.json","native-deep-cache","cocos-deep.json","flutter-deep.json","deep-gameplay.json",
