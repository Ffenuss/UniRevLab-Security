...
public class DeepEvidenceActivity extends AppCompatActivity {
    ...
    private String state(JSONObject value){if(value==null)return "нет отчёта";if(!value.optString("error","").isEmpty())return "backend partial/error";if(value.optBoolean("available")||value.optBoolean("detected")||value.optInt("analyzedLibraryCount")>0)return "evidence найден";return "не обнаружено";}
    private String value(JSONObject o,String key){if(o==null||!o.has(key)||o.isNull(key))return "—";Object v=o.opt(key);return v==null||v==JSONObject.NULL?"—":String.valueOf(v);}
    private String countIfReport(JSONObject report,long value){return report==null?"—":String.valueOf(value);}
    private String rva(Object value){if(value==null||value==JSONObject.NULL)return "";if(value instanceof Number)return "0x"+Long.toHexString(((Number)value).longValue());String s=String.valueOf(value).trim();if(s.isEmpty())return "";if(s.startsWith("0x")||s.startsWith("0X"))return "0x"+s.substring(2).toLowerCase(java.util.Locale.ROOT);try{return "0x"+Long.toHexString(Long.parseLong(s));}catch(Exception ignored){return s;}}
    ...
    @Override public void onCreate(Bundle state){super.onCreate(state);app=(App)getApplication();ScrollView scroll=new ScrollView(this);root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setPadding(dp(16),dp(18),dp(16),dp(30));root.setBackgroundColor(bg());scroll.addView(root);setContentView(scroll);
        TextView title=text("Deep evidence",29);title.setTypeface(null,Typeface.BOLD);root.addView(title);TextView note=text("Сводка встроенных backend'ов. Exact RVA означает подтверждённый статический адрес в бинарнике; structural/indirect evidence не выдаётся за runtime-наблюдение. Отсутствующие поля показываются как «—», настоящий 0 остаётся нулём, а ошибка backend не выдаётся за «ничего не найдено». Нажмите находку, чтобы увидеть исходный JSON evidence.",13);note.setTextColor(muted());root.addView(note);
