...
object CustomerReportExporter {
    ...
    private fun exportRussian(report: StaticAnalysisReport, gradleEvidence: GradleModuleEvidenceResult? = null, realDump: RealIl2CppDumpEngine.Result? = null): String {
        ...
        appendLine("- `offset-evidence.json` — подтверждённые method RVA и field offsets по всем успешно обработанным ABI; для каждой записи указаны namespace, класс, член, тип, сигнатура, формула вычисления адреса и требуемое runtime-основание. Гипотезы исключены.")
        ...
        appendLine("- `customer-action-map.json` — карта передачи заказчику: для каждого риска указано, что взять, из какого отчёта, что изменить в исходном продукте и как проверить исправление; эксплуатационные патчи и хуки не включаются.")
        appendLine("- `mod-resistance-validation.md` — подробный пошаговый playbook проверки в исходниках владельца и QA-сборке: доказательства, безопасная имитация недоверенного результата, исправления и критерии приёмки без создания рабочего мода.")
    ...
    private fun exportEnglish(report: StaticAnalysisReport, gradleEvidence: GradleModuleEvidenceResult?, realDump: RealIl2CppDumpEngine.Result?): String {
        ...
        return buildString {
            ...
            appendLine("- `customer-action-map.json`: evidence-to-remediation handoff map; it contains no executable patches, hooks, or bypass instructions.")
            appendLine("- `mod-resistance-validation.md`: owner-source QA playbook with evidence, safe outcome simulation, remediation and acceptance criteria; it does not create an operational mod.")
            appendLine()
