...
object RuntimeProfileScanner {
    ...
    fun scanApk(
        ...
        if (il2cpp?.detected != true) add("UNITY_MONO", buildList { if(libs.any{it.contains("libmono")}) add("libmono"); if(entries.any{it.contains("/managed/")&&it.endsWith(".dll")}) add("managed-dlls"); if(libs.any{it.endsWith("libunity.so")}) add("libunity.so") })
